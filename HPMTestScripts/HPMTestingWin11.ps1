...
$images = @([Image]@{Publisher='MicrosoftWindowsDesktop';Offer='office-365';Sku="win11-22h2-avd-m365";Version="22621.1992.230711"})
# $SharedImageResourceID = "/subscriptions/0325dd32-ebf8-403e-86b0-365c7d3306d9/resourceGroups/HPMBugBashRG/providers/Microsoft.Compute/galleries/HPMSharedImages/images/VMImageDef1/versions/1.0.0"
# $ManagedImageResourceID = "/subscriptions/0325dd32-ebf8-403e-86b0-365c7d3306d9/resourceGroups/HPMBugBashRG/providers/Microsoft.Compute/images/HPMBugBashManagedImage"
    ...
    for($j = 0; $j -le $VMSizes.Count; $j++) {
        ...
        for($k = 0; $k -le $disks.Count; $k++) {
             ...
             if($k -eq 0 -or ($k % 2 -eq 0)) {
                ...
                # if($j % 2 -eq 0) {
                #     # Use custom managed image
                #     Write-Host "Using custom managed image to update session host configuration"
                #     Update-AzWvdSessionHostConfiguration -SubscriptionId $subID -ResourceGroupName $ResourceGroupName -HostPoolName $HostPoolName -DiskInfoType $disks[$k] -VMSizeId $VMSizes[$j] -DomainInfoJoinType 'AzureActiveDirectory' -ImageInfoType 'Custom' -CustomInfoResourceId $ManagedImageResourceID
                # } 
                # else {
                ...
                # if($j % 2 -eq 0) {
                #     # Use custom shared image
                #     Write-Host "Using custom shared image to update session host configuration"
                #     Update-AzWvdSessionHostConfiguration -SubscriptionId $subID -ResourceGroupName $ResourceGroupName -HostPoolName $HostPoolName -DiskInfoType $disks[$k] -VMSizeId $VMSizes[$j] -DomainInfoJoinType 'AzureActiveDirectory' -ImageInfoType 'Custom' -CustomInfoResourceId $SharedImageResourceID
                # } 
                # else {
                Write-Host "Publisher: $publisher"
                Write-Host "SKU: $sku"
                ...
                Invoke-AzWvdInitiateSessionHostUpdate -HostPoolName $HostPoolName -ResourceGroupName $ -SubscriptionId $subID -UpdateDeleteOriginalVM -ScheduledDateTime $updatedTimeFormatted
             }
