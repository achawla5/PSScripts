...
try {
  ...
  if ($osBuild -ge 22000 -and (Get-Command -Name Copy-UserInternationalSettingsToSystem -ErrorAction SilentlyContinue)) {
    ...
    Write-Host "*** AVD AIB CUSTOMIZER PHASE: Set default Language - Pre-Windows 11 detected (build $osBuild). Applying registry-based fallback for default user region ***"
